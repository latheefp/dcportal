...
function testbed(){
  $schema = $this->Server->schema();
  //echo debug($schema);
  //echo debug($this->fieldUserInfo['serial']);
  $data = "AKD0949104";
  $oldData="AKD0949105";
  $output = $this->Field->showField($data,$this->fieldUserInfo['serial']);
  //echo debug ($data);
  //echo 
  $id=32;
  $name ="serial";
 ...
 if($oldData!=$data) {
     echo debug("Audit");
        $this->loadModel('Audit');
        $this->Audit->auditThis('Edit',$this->fieldUserInfo['serial']['title'].' from "' . $oldData . '" To "'.$data .'"',$this->name,$this->Session->read('User.id'),$id);
        $this->_alertChanges($hostname,false,$oldData,$data,$this->fieldUserInfo[$name]['title']);
    }
