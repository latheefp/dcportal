...
<table id="tblServers"><thead><tr><th>*</th>
            ...
            foreach($fieldUserInfo as $name=>$val) {
              //  echo debug($name);
                if($val['show']=='1') {
                    ...
                    if($val['sortable']=='1') {
                        print $this->Paginator->sort($name, $val['title']);
                    }else {
                        print $val['title'];
                    }
                    print '</th>';
    ...
    <tbody>
        ...
        foreach($data as $row) {
            ...
            $count++;
            print '>';
           // print '<td><a href="#" class="serverDetails" rowid="'.$row['Server']['id'].'"><img src="'.$this->Html->url('/img/notes.png').'" border=0></a></td>';
            print '<td  rowid='. $row['Server']['id']. '>';
            //print $this->Html->link(array('controller'=>'servers', 'action'=>'servers/details')) . '</td>';
            echo $this->Html->link($this->Html->image('/img/notes.png'), '/servers/detail/'.$row['Server']['id'], array('escape' => false));
            print '</td>';
            foreach($row as $table) {
               foreach ($order as $key=>$val){
                    //echo debug($val . "   " . $table[$val]);
                    //echo debug($fieldUserInfo[$val]);
...
<div id="dialog-addnew" style="display:none;" class="dialog-addnew" title="Add new server">
    <form id="addnew-form" method="post" class="addnew-form">
        <table id="addnew" class="addnew">
            <tbody></tbody>
        </table>    
    </form>
</div>
...
<div id="dialog-serverDetails"  style="display:none;" title="Server details">
    <div id="serverDetailstabs">
        <ul>
            <li><a id="-tab2" href="#tabs2">Audit</a></li>
        </ul>
        <div id="tabs-1"></div>
        <div id="tabs-2"></div>
        <div id="tabs-3"></div>
...
//           // $("#add-body").append('<input type="text"></input>');
                ...
                $("#add-body").append('Host name: <input type="text" id=hostname name=hostname>');
//                $("#add-body").append('Location: <input type="select" id=location name=location>');
//                $("#add-body").append('Rack: <input type="text" id=rack name=rack>');
//                $("#add-body").append('Serial: <input type="text" id=serial name=serial>');
//                $("#add-body").append('OS: <input type="Select" id=os name=os>');
//                $("#add-body").append('OS Version: <input type="text" id=os_version name=os_version>');
//                $("#add-body").append('Serial: <input type="text" id=serial name=serial>');
//                $("#add-body").append('Project: <input type="select" id=project name=project>');
//                $("#add-body").append('Serial: <input type="text" id=serial name=serial>');
//                 
...
//echo ($subaction);
 ?>
    ...
    function editRow(myTD){
        ...
        //alert (myTD);
         //alert(myTD + " " + index + " " + myType );
         //alert(myTD.cellIndex);
...
$("#dialog-addnew").dialog({
                    ...
                    buttons: {
                        'Add': function() {
                            var data = $( "form#addnew-form" ).serialize();
                            //console.log(str);
                            //alert(data);
                           // if (validateForm() ==true){
                            if (validateForm() ==true){
                             $.ajax({
                                ...
                                async: false,
                                data:data,
                                url: "<?php echo $this->Html->url("/servers/AddNewHost/"); ?>", //+data,
                                success: function(msg){
                                window.location = '<?php echo '/Servers/'; ?>?qsearch='+ $('#hostname-addnew').val();
                                    //$('#testdata').html(msg);
                                }
                                });
                                 $(this).dialog('close');
                    ...
                    open: function() {
                            $('.ui-dialog-buttonpane').find('button:contains("Add")').attr("id", "AddNewAddButtton");
                           }
                });
 <?php
  $rows="";
     ...
     if($val['mandate']=='1') {
           ...
           switch ($mytype) {
                case "string":
                        $type="text";
                        ?>
                        $('#addnew tbody').append('<?php print "<tr><td id=label>". $title ."</td><td id=inputdata>";?>  <input type= <?php print $type; ?> id=<?php print $name . "-addnew"; ?> validate=false name=<?php print $name; ?> maxlength=<?php print $length . " field=" .$name. "></td></tr>"; ?>');
                        <?php                                                      
                        break;
                ...
                case "select":
                       ...
                       // $list=explode(",", $val['extra']);
                        //print $list;
                        ?>
                        $('#addnew tbody').append('<?php print "<tr><td id=label>". $title ."</td><td id=inputdata> <select id=" . $name."-addnew  validate=false field=" .$name.  " name=" .$name;
                 ...
                 case "multiselect":
                       ...
                       // $list=explode(",", $val['extra']);
                        //print $list;
                        ?>
                        $('#addnew tbody').append('<?php print "<tr><td id=label>". $title ."</td><td id=inputdata> <select multiple id=" . $name ."-addnew  validate=false field=" .$name.  " name=" .$name."[]";
                        if( isset($list[0])){$select = $list[0];}
                        //$options=' <option selected="selected"> '. $select .'</option>';
                        $options=' <option selected="selected"> '. $select .'</option>';
                        foreach($list as $key => $value):
...
$('#dialog-addnew').children().focusout(function(event){
                        var myid=(event.target.id)
                        var myfield=$('#'+myid).attr('field');
                        var myval=($('#'+myid).val());
                         $.post(
                         '/servers/validate_form', 
                         {field:myfield, value:myval},
                         handleValidation);
                         function handleValidation(error){
                             if(error.length != 0){
                             $('#'+myid).attr('validate', "false");
                             if (document.getElementById(myid+'validate')){
                                $('#'+ myid +'validate').remove();
                                $('#'+myid).after('<div id=' + myid + 'validate' + ' class="addnewerror-message">' + error + "</div>") ;
                                }else{
                                    $('#'+myid).after('<div id=' + myid + 'validate' + ' class="addnewerror-message">' + error + "</div>") ;
                             }
                         }
    ...
    function validateForm(){
        var result=true;
        $(':input','#dialog-addnew').each(function(){
            var validate=($(this).attr('validate'));
            var myid=($(this).attr('id'));
            if (validate=="false"){
                //alert("Still False=" + myid + validate);
                result=false;
              }
             });
           if (result==false){
//               if (!document.getElementById('ValSataus')){
//                    $('#addnew').after('<div id=ValSataus' + ' class="addnewerror-message"> Please fill all Details </div>');
//               }
//           }else{
//               $('#ValSataus').remove();
            ...
            function handleValidation( myfield, myval, myid){
               // alert(myfield);
                $.post(
                '/servers/validate_form',
                {field:myfield, value:myval})
                .done(function( error ) {
             //           alert ("Processing error label"+ error + myid);
                        if(error.length != 0){
...
<?php
//$JscriptValidate = '<script type="text/javascript">
//    $(document).ready(function(){
//        $(\'#hostname-addnew\').blur(function(){
//                         $.post(
//                         \'/servers/validate_form\', 
//                         {field:$(\'#hostname-addnew\').attr(\'id\'), value:$(\'#hostname-addnew\').val()},
//                         handleNameValidation);
//                     });
//                     function handleNameValidation(error){
//                         if(error.length != 0){
//                             if (!document.getElementById("hostname-notEmpty")){
//                                var ErrorWith=$("#hostname-addnew").width();
//                                $(\'#hostname-addnew\').after(\'<div  style="width=\'+ ErrorWith + \'px;" id="hostname-notEmpty" class="addnewerror-message">\' + error + "</div>") ;
//                             }
//                         }
//                         else
//                           $(\'#hostname-notEmpty\').remove();
//                        }
//                    });
//                    </script>';

//echo $this->addScript($JscriptValidate, false); //add script to header
?>